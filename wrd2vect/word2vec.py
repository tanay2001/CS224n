...
def naiveSoftmaxLossAndGradient(
    ...
    l = y_hat.copy()
    l[outsideWordIdx] -=1
    gradCenterVec = np.dot(outsideVectors.T, l)

    gradOutsideVecs = np.dot(l[:,np.newaxis], centerWordVec[np.newaxis,:])
...
def negSamplingLossAndGradient(
    ...
    ### YOUR CODE HERE
    uovc = np.dot(outsideVectors[outsideWordIdx].T, centerWordVec)
    negative_samples = outsideVectors[negSampleWordIndices]
    ukvc = negative_samples.dot(centerWordVec)* -1

    ### Please use your implementation of sigmoid in here.
    loss = -np.log(sigmoid(uovc))

    loss +=  - np.sum(np.log(sigmoid(ukvc)))

    gradCenterVec = -(1 - sigmoid(uovc)) * outsideVectors[outsideWordIdx] + np.sum(np.expand_dims(1 - sigmoid(ukvc), axis=1) * negative_samples, axis=0)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in outsideWords:
        ind  = word2Ind[i] 
    one_loss, GradC, GrdaO= word2vecLossAndGradient(vc,ind, outsideVectors,dataset)

    loss += one_loss
    gradCenterVecs[c] += GradC

    gradOutsideVectors += GrdaO
